...
RUNDIR="${EXPDIR}/RUNS/${1}/FALL3D-GFS"
...
if [ -d $RUNDIR ]; then
    echo "FALL3D-GFS RUN Folder found! Closing..."
    exit 1
else
    echo "****************************"
...
sed -e "1,/ENDDEF/s|\${RUNDIR}|${RUNDIR}|"            \
    ...
    ${EXPDIR}/templates/FALL3D-GFS_PRE.cmd > ${PRJDIR}/SCRIPTS/FALL3D-GFS_PRE.cmd
...
sed -e "1,/ENDDEF/s|\${RUNDIR}|${RUNDIR}|"            \
    ...
    ${EXPDIR}/templates/FALL3D-GFS.cmd > ${PRJDIR}/SCRIPTS/FALL3D-GFS.cmd
...
sed -e "1,/ENDDEF/s|\${RUNDIR}|${RUNDIR}|"            \
    ...
    -e "1,/ENDDEF/s|\${NENS}|${NENS}|"                \
    ${EXPDIR}/templates/FALL3D-GFS_POS.cmd > ${PRJDIR}/SCRIPTS/FALL3D-GFS_POS.cmd
